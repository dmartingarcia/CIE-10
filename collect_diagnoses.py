...
def iterate_through_codes():
    for first_char in ascii_uppercase:
...
with open('csv/cie10-es.csv', 'w', newline='') as csvfile:
...
for entry in progressbar(codes, redirect_stdout=True):
    ...
    r = requests.put(f"http://localhost:9200/diagnoses_{epoch}/_doc/{entry}", data=json.dumps(code), headers=headers)
    i = i +1
